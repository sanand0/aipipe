...
export const providers = {
  ...
  openai: {
    ...
    cost: async ({ model, usage, body }) => {
      // If the API response model is not in our pricing list (e.g. a specific snapshot version like gpt-5.4-nano-2026-03-17),
      // we fallback to the model name requested by the user, which has to be in pricing.json.
      if (model && !openaiCost[model] && body) {
        try {
          const reqModel = JSON.parse(body).model;
  ...
  geminiv1beta: {
    ...
    cost: async ({ model, usage, env, path, body }) => {
      ...
      // If the API response model is not in our pricing list (e.g. an unknown snapshot),
      // we fallback to the model name requested by the user, which has to be in pricing.json.
      if (model && !geminiCost[model]) {
        let reqModel;
        if (body) {
          try {
        ...
        if (reqModel && geminiCost[reqModel]) {
          model = reqModel;
        }
      }
